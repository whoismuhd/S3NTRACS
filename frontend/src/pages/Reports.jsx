...
export default function Reports() {
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
          ...
          <p className="mt-1 text-sm text-gray-500">Run a scan to generate a compliance report.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Compliance Mapping */}
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">Compliance Framework Mapping</h2>
        <div className="space-y-4">
          {Object.entries(report.compliance_mapping || {}).map(([framework, findings]) => (
            <div key={framework} className="border border-gray-200 rounded-lg p-4">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-base font-semibold text-gray-900">{framework}</h3>
                <Badge variant="info" size="sm">
                  {findings.length} finding{findings.length !== 1 ? 's' : ''}
                </Badge>
              </div>
              {findings.length === 0 ? (
                <p className="text-sm text-gray-500">No findings mapped to this framework</p>
              ) : (
                  ...
                  {findings.slice(0, 5).map((finding, idx) => (
                    <div key={idx} className="flex items-start space-x-2 text-sm">
                      ...
                      <div className="flex-1">
                        <span className="font-medium text-gray-700">{finding.control}:</span>
                        <span className="text-gray-600 ml-1">{finding.title}</span>
                      </div>
                    </div>
                  ))}
                  {findings.length > 5 && (
